      ...
      <div className="relative aspect-video w-full overflow-hidden rounded-panel border border-hairline bg-panel-raised">
        <AnimatePresence mode="wait">
          {showTranscript ? (
            ...
            >
              {!loadingTranscript && rangeTranscript.length > 0 && (
                  ...
                  filteredTranscript.length > 0 ? (
                    <>
                      {filteredTranscript.map((l, i) => (
                        <div
                          key={`${l.start}-${i}`}
                          className="group flex items-start gap-3 rounded-row px-2 py-1.5 transition-colors hover:bg-panel-hover"
                        >
                          <span className="w-14 shrink-0 pt-[1px] text-[11px] tabular-nums text-fg-faint">
                            {formatTimestamp(l.start)}
                          </span>
                          <span className="min-w-0 flex-1 text-[13px] leading-relaxed text-fg">
                            {l.text}
                          </span>
